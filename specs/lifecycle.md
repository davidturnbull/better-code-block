# better-code-block - Lifecycle

This document describes the lifecycle of the better-code-block plugin, from initialization to cleanup, explaining how the plugin operates during different phases of Vim usage.

## Plugin Lifecycle Overview

The lifecycle of the better-code-block plugin can be divided into several distinct phases:
...
When Vim starts, it loads plugins in a specific order. The better-code-block plugin's loading sequence is:

### 1.1. Plugin Script Loading

The `plugin/better_code_block.vim` file is loaded first, which:

- Checks if the plugin is already loaded via `g:loaded_better_code_block`
...
- Sets up the highlight style by calling `better_code_block#setup_highlight_style()`
...
If the current buffer has a supported file extension, the plugin immediately calls `better_code_block#apply_highlighting()` to apply highlighting to any code blocks in the current buffer.
...
The `ftplugin/markdown/better_code_block.vim` file is loaded, which:

- Checks if the plugin is already loaded for this buffer via `b:loaded_better_code_block`
- Initializes buffer-specific variables like `b:highlighting_enabled`
- Applies initial highlighting by calling `better_code_block#apply_highlighting()`
...
- User commands (`:BetterCodeBlockRefresh`)
- Plugin functions (`better_code_block#enable()`, `better_code_block#toggle()`)

### 3.2. Debouncing (Optional)

If `g:better_code_block_update_delay` is greater than 0, the plugin debounces rapid changes:

- Cancels any pending update timer
- Starts a new timer that will call `better_code_block#do_apply_highlighting()` after the specified delay

### 3.3. Highlight Application

The `better_code_block#do_apply_highlighting()` function:

1. Clears previous highlights by calling `better_code_block#clear_highlights()`
...
The `better_code_block#parse_highlight_spec()` function:
...
The `better_code_block#toggle()` function:

1. Checks the current state of `b:highlighting_enabled`
2. Toggles the state
3. Calls either `better_code_block#enable()` or `better_code_block#disable()`

### 4.2. Changing Highlight Style

The `better_code_block#change_highlight_style()` function:

1. Updates `g:better_code_block_style` with the new style
2. Calls `better_code_block#setup_highlight_style()` to apply the new style
3. Refreshes highlighting with `better_code_block#apply_highlighting()`

### 4.3. Toggling Line Numbers

The `better_code_block#toggle_line_numbers()` function:

1. Toggles `g:better_code_block_show_line_numbers`
2. Calls appropriate functions to enable or disable line numbers
3. Refreshes highlighting

### 4.4. Registering Custom Styles

The `better_code_block#register_custom_style()` function:

1. Parses style attributes from the command arguments
2. Adds the new style to `g:better_code_block_custom`
...
1. Calls `better_code_block#clear_highlights()` to remove all highlighting
...
| -------------- | ------------------------------------------- | ----------------------------------------------------- |
| `BufReadPost`  | `better_code_block#apply_highlighting()`    | Initial highlighting after buffer load                |
| `BufWritePost` | `better_code_block#apply_highlighting()`    | Update highlighting after save                        |
| `InsertLeave`  | `better_code_block#apply_highlighting()`    | Update highlighting after exiting insert mode         |
| `TextChanged`  | `better_code_block#apply_highlighting()`    | Update highlighting after text changes in normal mode |
| `TextChangedI` | `better_code_block#apply_highlighting()`    | Update highlighting after text changes in insert mode |
| `CursorMovedI` | `better_code_block#apply_highlighting()`    | Update highlighting when cursor moves in insert mode  |
| `BufLeave`     | `s:cleanup()`                               | Clean up when leaving buffer                          |
| `BufUnload`    | `better_code_block#clear_highlights()`      | Clear highlights when buffer is unloaded              |
| `ColorScheme`  | `better_code_block#setup_highlight_style()` | Update highlight styles when colorscheme changes      |

## Resource Management

The plugin carefully manages resources to prevent memory leaks and ensure clean operation:

1. **Match IDs** - Stored in `w:better_code_block_match_ids` and cleared when needed
2. **Error Match IDs** - Stored in `w:better_code_block_error_match_ids` and cleared when needed
